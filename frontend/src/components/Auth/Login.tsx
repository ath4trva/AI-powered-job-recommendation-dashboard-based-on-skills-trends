...
const Login = ({ onLogin }: LoginProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log("User Authenticated:", { email, name: isSignUp ? name : 'User' });
  ...
  return (
    <div className="flex justify-center items-center h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="bg-white rounded-lg p-10 w-full max-w-md shadow-lg border border-slate-200">
        <div className="mb-8">
          <h2 className="text-3xl font-bold text-primary mb-2">
            {isSignUp ? 'Join Us' : 'Welcome Back'}
          </h2>
          <p className="text-text-secondary text-sm">
            {isSignUp ? 'Create your professional profile' : 'Login to your account'}
          </p>
        </div>
